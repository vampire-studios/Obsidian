...
public final class CrossbowShotProjectileEvents {
	...
	public static final Event<ModifyProjectileFromCrossbow> CROSSBOW_MODIFY_SHOT_PROJECTILE = Event.create(ModifyProjectileFromCrossbow.class, callbacks -> (bowStack, projectileStack, user, entity) -> {
		for (ModifyProjectileFromCrossbow callback : callbacks) {
			callback.modifyProjectileShot(bowStack, projectileStack, user, entity);
		}
	});
	 ...
	 */
	public static final Event<ReplaceProjectileFromCrossbow> CROSSBOW_REPLACE_SHOT_PROJECTILE = Event.create(ReplaceProjectileFromCrossbow.class, callbacks -> (bowStack, projectileStack, user, entity) -> {
		for (ReplaceProjectileFromCrossbow callback : callbacks) {
			PersistentProjectileEntity replacedEntity = callback.replaceProjectileShot(bowStack, projectileStack, user, entity);
	...
	public interface ReplaceProjectileFromCrossbow {
		 ...
		 * @param entity					the arrow entity to be spawned
		 * @return the new projectile entity. Return {@code null} if you do not change the entity.
		 */
		PersistentProjectileEntity replaceProjectileShot(ItemStack crossbowStack, ItemStack projectileStack, LivingEntity user, @NotNull PersistentProjectileEntity entity);
	}
	...
	public interface ModifyProjectileFromCrossbow {
		 ...
		 * @param entity 					the arrow entity to be spawned
		 */
		void modifyProjectileShot(ItemStack crossbowStack, ItemStack projectileStack, LivingEntity user, @NotNull PersistentProjectileEntity entity);
	}
}