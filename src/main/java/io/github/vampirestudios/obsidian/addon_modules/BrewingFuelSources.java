...
import io.github.vampirestudios.obsidian.api.obsidian.BrewingFuelSource;
...
public class BrewingFuelSources implements AddonModule {

	@Override
	public void init(IAddonPack addon, File file, BasicAddonInfo id) throws IOException {
		BrewingFuelSource fuelSource = AddonFormats.read(addon, file, BrewingFuelSource.class);
		...
		try {
			...
			DefaultItemComponentEvents.MODIFY.register(context ->
					context.modify(predicate, (builder, item) -> {
						if (json.entrySet().isEmpty() || fuelSource.uses == null) {
							builder.set(DataComponents.BREWING_FUEL, null);
							return;
						}

						builder.set(DataComponents.BREWING_FUEL, new BrewingFuel(fuelSource.uses, fuelSource.speed_multiplier));
					})
			);

			register(
					ContentRegistries.BREWING_FUEL_SOURCES,
					"brewing_fuel_source",
					Identifier.fromNamespaceAndPath(id.modId(), AddonFormats.baseName(file)),
					fuelSource
			);
		} catch (Exception e) {
			failedRegistering("brewing_fuel_source", file.getName(), e);
		}
	}

	@Override
	public String getType() {
		return "fuel_source/brewing";
	}
}