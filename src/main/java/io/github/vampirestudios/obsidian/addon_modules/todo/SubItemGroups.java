...
public class SubItemGroups implements AddonModule {
	...
	public void init(IAddonPack addon, File file, BasicAddonInfo id) throws IOException {
		SubItemGroup itemGroup = AddonFormats.read(addon, file, SubItemGroup.class);

		try {
			if (itemGroup == null) return;
			CreativeModeTab tab = BuiltInRegistries.CREATIVE_MODE_TAB.getValue(itemGroup.targetGroup);

			Identifier tabId = Identifier.fromNamespaceAndPath(id.modId(), AddonFormats.baseName(file));
			itemGroup.id = tabId;
