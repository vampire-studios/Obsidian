	...
	public void init(IAddonPack addon, File file, BasicAddonInfo id) throws IOException, SyntaxError {
		...
		try {
			...
			if (biomeLayout.dimensionType.equals(BiomeLayout.DimensionType.OVERWORLD)) {
				for (BiomeLayout.BiomeInformation biome : biomeLayout.biomes) {
					BiomePlacement.addOverworld(ResourceKey.create(Registries.BIOME, biome.name), new Climate.ParameterPoint(
							biome.multiNoise.temperatureNoise.getValue(),
							biome.multiNoise.humidityNoise.getValue(),
							biome.multiNoise.continentalnessNoise.getValue(),
							biome.multiNoise.erosionNoise.getValue(),
							biome.multiNoise.depthNoise.getValue(),
							biome.multiNoise.weirdnessNoise.getValue(),
							(long) biome.multiNoise.offset
					));
				}
			}
			if (biomeLayout.dimensionType.equals(BiomeLayout.DimensionType.NETHER)) {
				for (BiomeLayout.BiomeInformation biome : biomeLayout.biomes) {
					BiomePlacement.addNether(ResourceKey.create(Registries.BIOME, biome.name), new Climate.ParameterPoint(
							biome.multiNoise.temperatureNoise.getValue(),
							biome.multiNoise.humidityNoise.getValue(),
							biome.multiNoise.continentalnessNoise.getValue(),
							biome.multiNoise.erosionNoise.getValue(),
			...
			if (biomeLayout.dimensionType.equals(BiomeLayout.DimensionType.END)) {
				for (BiomeLayout.BiomeInformation biome : biomeLayout.biomes) {
					BiomePlacement.addEnd(ResourceKey.create(Registries.BIOME, biome.name), new Climate.ParameterPoint(
							biome.multiNoise.temperatureNoise.getValue(),
							biome.multiNoise.humidityNoise.getValue(),
							biome.multiNoise.continentalnessNoise.getValue(),
							biome.multiNoise.erosionNoise.getValue(),
...
//				}
//			});
//			register(ContentRegistries.BIOME_LAYOUTS, "biome_layout", biomeLayout.regionName, biomeLayout);
		} catch (Exception e) {
