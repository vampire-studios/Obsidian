...
//            Item registeredItem;
            if (item.information.canPlaceBlock) {
                REGISTRY_HELPER.items().registerItem(identifier.getPath(), new BlockItemImpl(item, BuiltInRegistries.BLOCK.get(item.information.placableBlock),
                        settings));
            } else {
                ...
                if (item.information.wearable) {
                    ...
                    if (item.information.dyeable) {
                        expanded.registerDyeableItem(new WearableAndDyeableItemImpl(item, settings), identifier.getPath(), item.information.getItemGroup());
                    } else {
                        expanded.registerItem(identifier.getPath(), new WearableItemImpl(item, settings), item.information.getItemGroup());
                    }
                } else {
                    if (item.information.dyeable) {
                        expanded.registerDyeableItem(new DyeableItemImpl(item, settings), identifier.getPath(), item.information.getItemGroup());
                    } else {
                        expanded.registerItem(identifier.getPath(), new ItemImpl(item, settings), item.information.getItemGroup());
                    }
                }
            }
