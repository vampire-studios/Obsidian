...
public class LegacyObsidianAddon implements IAddonPack {
	...
	public PackResources getVirtualResourcePack() {
		...
		if (file.getParentFile().isDirectory())
			return new PathPackResources(packLocationInfo, file.getParentFile().toPath());
		else
			return new FilePackResources(packLocationInfo, new FilePackResources.SharedZipFileAccess(this.file), obsidianAddonInfo.namespace);
	}
