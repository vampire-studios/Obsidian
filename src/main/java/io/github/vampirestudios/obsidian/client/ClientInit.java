...
import io.github.vampirestudios.obsidian.client.palette.PaletteTintSource;
import io.github.vampirestudios.obsidian.client.renderer.RotationBlockRenderer;
...
public class ClientInit implements ClientModInitializer {
	...
	private static void generateCreativeTabTranslations(String id) {
		ContentRegistries.CREATIVE_TABS.entrySet().forEach(entry -> {
			Identifier tabId = entry.getKey().identifier();
			if (!tabId.getNamespace().equals(id)) return;
			addTabTranslations(tabId, entry.getValue().nameInformation);
		});

		ContentRegistries.ITEM_GROUPS.entrySet().forEach(entry -> {
			Identifier tabId = entry.getKey().identifier();
			if (!tabId.getNamespace().equals(id)) return;
			addTabTranslations(tabId, entry.getValue().name);
		});
	}

	private static void addTabTranslations(Identifier tabId, NameInformation name) {
		if (name == null) return;

		String key = "itemGroup." + tabId.toLanguageKey();
		if (name.translations != null && !name.translations.isEmpty()) {
			name.translations.forEach((languageId, translation) ->
					addTranslation(tabId.getNamespace(), languageId, key, translation));
		} else if (name.text != null && !name.text.isEmpty()) {
			addTranslation(tabId.getNamespace(), "en_us", key, name.text);
		}
	}
...
//        ContentPackSyncNetworking.registerClientReceivers();
		...
		EntityRenderers.register(Obsidian.PROJECTILE, ThrownItemRenderer::new);
		EntityRenderers.register(Obsidian.PROJECTILE_ARROW, io.github.vampirestudios.obsidian.client.renderer.ProjectileArrowRenderer::new);

		BlockEntityRendererRegistry.register(OBE.ROTATION_BLOCK, RotationBlockRenderer::new);
		ObsidianAddonLoader.OBSIDIAN_ADDONS.forEach(iAddonPack -> {
			...
			if (!iAddonPack.getConfigPackInfo().hasAssets) {
				...
				SidedPackwrightCallback.BETWEEN_MODS_AND_USER.register((type, packs) -> {
	...
	/**
	 * Runs one piece of asset generation, keeping its failure to itself.
	 *
	 * <p>Generation is a plain loop over every declared item, so an exception from any one of them used
	 * to abort the rest — every model, block state and translation after it in the pack. One malformed
	 * declaration should cost its own model, not the pack's.
	 */
	private static void generateSafely(Identifier what, Runnable generator) {
		try {
			generator.run();
		} catch (Exception e) {
			Obsidian.LOGGER.error("[Obsidian] Failed to generate assets for {}", what, e);
		}
	}
	...
	private static void generateAssets(String id, RuntimeResourcePack resourcePack, boolean registerBlockColors) {
		...
		for (NexoItem item : ContentRegistries.NEXO_ITEMS)
			if (item.id.getNamespace().equals(id))
				generateSafely(item.id, new OraxenItemInitThread(resourcePack, item));
		...
		for (ArmorItem armor : ContentRegistries.ARMORS)
			if (armor.information.id.getNamespace().equals(id))
				generateSafely(armor.information.id, new ArmorInitThread(resourcePack, armor));
		for (Elytra elytra : ContentRegistries.ELYTRAS)
			if (elytra.information.id.getNamespace().equals(id))
				generateSafely(elytra.information.id, new ElytraAssetsThread(resourcePack, elytra));
		generateCreativeTabTranslations(id);
