...
public class AssetJsonGenerator extends JsonGenerator {
	...
	public void blockState(Identifier id, Consumer<VariantBlockStateGenerator> consumer) {
		...
		consumer.accept(gen);
		json(Identifier.fromNamespaceAndPath(id.getNamespace(), "blockstates/" + id.getPath()), gen.toJson());
	}
	...
	public void blockModel(Identifier id, Consumer<ModelGenerator> consumer) {
		...
		consumer.accept(gen);
		json(Identifier.fromNamespaceAndPath(id.getNamespace(), "models/block/" + id.getPath()), gen.toJson());
	}

	public void itemModel(Identifier id, Consumer<ModelGenerator> consumer) {
		ModelGenerator gen = new ModelGenerator();
		consumer.accept(gen);
		json(Identifier.fromNamespaceAndPath(id.getNamespace(), "models/item/" + id.getPath()), gen.toJson());
	}
}