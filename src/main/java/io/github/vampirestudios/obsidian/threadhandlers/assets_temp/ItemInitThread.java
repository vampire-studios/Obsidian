...
public class ItemInitThread implements Runnable {
	...
	public void run() {
		...
		if (item.rendering != null) {
			...
			if (isCrossbow) {
				...
				// "when": "arrow" — a bare value, not an array: SelectCase encodes primitives and
				// lists, and a Java array is neither.
				if (arrowInfo != null) {
					chargeType.addCase(SelectCase.of("arrow", ItemModel.model(arrowId)));
				}

				// "when": "rocket"
				if (rocketInfo != null) {
					chargeType.addCase(SelectCase.of("rocket", ItemModel.model(rocketId)));
				}
			...
			// fishing_rod/cast is a boolean, so it belongs in a condition — a select would both
			// fail to encode and mean nothing to the model parser.
			...
			if (castInfo != null) {
				...
				model = new ModelCondition()
						.property(PropertyFishingRodCast.fishingRodCast())
						.onTrue(ItemModel.model(castId))
						.onFalse(model);
			}

			// 5) Trident throwing (optional) — also a boolean property.
			...
			if (throwingInfo != null) {
				...
				model = new ModelCondition()
						.property(PropertyThrowing.throwing())
						.onTrue(ItemModel.model(throwingId))
						.onFalse(model);
			}
		}
		...
		if (dyeable) {
			...
			PaletteAssets.tintEveryModel(model, new TintDye(defaultColor));
		}

		PaletteAssets.applyTints(item, model);
	...
	private ItemModel buildBowUseDurationDispatch(Identifier itemId, io.github.vampirestudios.obsidian.api.obsidian.ItemDisplayInformation di) {
		...
		// use_duration counts ticks, so the scale is 1/draw time — a shortbow reaching full draw in
		// 10 ticks has to run through its pulling stages twice as fast as a vanilla bow.
		int drawTicks = item instanceof RangedWeaponItem rangedWeapon
				? rangedWeapon.drawTicks()
				: RangedWeaponItem.DEFAULT_DRAW_TIME;

		ModelRangeDispatch dispatch = (ModelRangeDispatch) new ModelRangeDispatch()
				.property(PropertyUseDuration.useDuration())
				.scale(1.0f / drawTicks)
