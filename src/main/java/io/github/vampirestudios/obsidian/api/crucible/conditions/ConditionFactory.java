...
public class ConditionFactory {
	public static Condition createCondition(String type, Map<String, Object> parameters) {
		...
		} else if (matchesAlias(type, List.of("hasAuraStacks", "hasaurastacks", "aura_stack_count", "aura_stack"))) {
			String auraId = (String) parameters.getOrDefault("aura", "");
			int minStacks = number(parameters, "stacks", 1);
			return new HasAuraStacksCondition(auraId, minStacks);
		} else if (matchesAlias(type, List.of("lowHunger", "lowhunger", "hunger", "hungerbelow", "low_hunger"))) {
			int minHunger = number(parameters, "hunger", 6);
			boolean applyToCaster = (boolean) parameters.getOrDefault("applyToCaster", true);
			return new LowHungerCondition(minHunger, applyToCaster);
		} else if (matchesAlias(type, List.of("targetWithin", "targetwithin", "nearby", "inrange", "proximity", "target_within"))) {
			return new TargetWithinDistanceCondition(number(parameters, "distance", 5));
		}

		// An unknown condition is the caller's to report — throwing here took down the whole skill.
		return null;
	}

	/** Condition parameters arrive as doubles, or as strings when they came from a non-numeric field. */
	private static int number(Map<String, Object> parameters, String key, int fallback) {
