		...
		for (EntityModel.Bone part : bones) {
			...
			for (EntityModel.Bone.Cube cube : part.cubes) {
				...
				if (cube.radiusArray != null)
					dilation.extend(cube.radiusArray.getX(), cube.radiusArray.getY(), cube.radiusArray.getZ());
			...
			if (part.parent != null && !part.parent.isEmpty() && !part.parent.isBlank())
				modelPartData.getChild(part.parent)
