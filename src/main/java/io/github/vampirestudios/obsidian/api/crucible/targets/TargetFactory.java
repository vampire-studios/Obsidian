...
public class TargetFactory {
	...
	static {
		...
		targets.add(new EntitiesAboveTarget(5, 3));                // Entities in a column overhead
		targets.add(new NearestAlliesTarget(3, 10));               // Nearest entities on the caster's side
		targets.add(new TargetLocationTarget());                   // Where the caster last struck
	}
		...
		for (SkillTarget<?> target : targets) {
			if (target.aliases.contains(key.toLowerCase())) {
				return switch (target) {
					...
					case RandomLocationsNearOriginTarget _ ->
							(SkillTarget<T>) createRandomLocationsNearOriginTarget(parameters);
					case RandomLocationsNearCasterTarget _ ->
							(SkillTarget<T>) createRandomLocationsNearCasterTarget(parameters);
					...
					case LocationsAboveEntitiesTarget _ ->
							(SkillTarget<T>) createLocationsAboveEntitiesTarget(parameters);
					case EntitiesAboveTarget _ -> (SkillTarget<T>) createEntitiesAboveTarget(parameters);
					case NearestAlliesTarget _ -> (SkillTarget<T>) createNearestAlliesTarget(parameters);
	...
	private static EntitiesAboveTarget createEntitiesAboveTarget(Map<String, String> parameters) {
		double radius = Double.parseDouble(parameters.getOrDefault("radius", "5"));
		double height = Double.parseDouble(parameters.getOrDefault("height", "3"));
		return new EntitiesAboveTarget(radius, height);
	}

	private static NearestAlliesTarget createNearestAlliesTarget(Map<String, String> parameters) {
		int amount = Integer.parseInt(parameters.getOrDefault("amount", "3"));
		double radius = Double.parseDouble(parameters.getOrDefault("radius", "10"));
		return new NearestAlliesTarget(amount, radius);
	}
