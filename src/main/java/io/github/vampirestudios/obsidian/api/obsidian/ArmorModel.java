	...
	public LayerDefinition getTexturedModelData() {
		...
		for (ArmorModel.Bone part : bones) {
			...
			for (ArmorModel.Bone.Cube cube : part.cubes) {
				...
				if (cube.radiusArray != null)
					dilation.extend(cube.radiusArray.x(), cube.radiusArray.y(), cube.radiusArray.z());
				else if (cube.radius != 0) dilation.extend(cube.radius);

				if (cube.name.isEmpty())
					modelPartBuilder.addBox(cube.offset.x(), cube.offset.y(), cube.offset.z(), cube.size.x(), cube.size.y(), cube.size.z(), dilation);
				else
					modelPartBuilder.addBox(cube.name, cube.offset.x(), cube.offset.y(), cube.offset.z(), cube.size.x(), cube.size.y(), cube.size.z(), dilation);
