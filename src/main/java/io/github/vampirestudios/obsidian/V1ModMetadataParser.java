	...
	public static LoaderModMetadata parse(JsonReader reader) throws IOException, ParseMetadataException {
		...
		while (reader.hasNext()) {
			...
			switch (key) {
				...
				case "provides" -> readProvides(reader, provides);
				case "environment" -> {
					if (reader.peek() != JsonToken.STRING) {
				...
				case "entrypoints" -> readEntrypoints(warnings, reader, entrypoints);
				case "jars" -> readNestedJarEntries(warnings, reader, jars);
				case "mixins" -> readMixinConfigs(warnings, reader, mixins);
				case "accessWidener" -> {
					if (reader.peek() != JsonToken.STRING) {
				...
				case "depends" -> readDependenciesContainer(reader, ModDependency.Kind.DEPENDS, dependencies);
				case "recommends" -> readDependenciesContainer(reader, ModDependency.Kind.RECOMMENDS, dependencies);
				case "suggests" -> readDependenciesContainer(reader, ModDependency.Kind.SUGGESTS, dependencies);
				case "conflicts" -> readDependenciesContainer(reader, ModDependency.Kind.CONFLICTS, dependencies);
				case "breaks" -> readDependenciesContainer(reader, ModDependency.Kind.BREAKS, dependencies);
				case "requires" -> {
					hasRequires = true;
					reader.skipValue();
				}
				...
				case "authors" -> readPeople(warnings, reader, authors);
				case "contributors" -> readPeople(warnings, reader, contributors);
				case "contact" -> contact = readContactInfo(reader);
				case "license" -> readLicense(reader, license);
				case "icon" -> icon = readIcon(reader);
				case "languageAdapters" -> readLanguageAdapters(reader, languageAdapters);
				case "custom" -> readCustomValues(reader, customValues);
				default -> {
	...
	private static void readPeople(List<ParseWarning> warnings, JsonReader reader, List<Person> people) throws IOException, ParseMetadataException {
		...
		while (reader.hasNext()) {
			switch (reader.peek()) {
				...
				case BEGIN_OBJECT -> {
					...
					while (reader.hasNext()) {
						...
						switch (key) {
							...
							case "contact" -> contactInformation = V1ModMetadataParser.readContactInfo(reader);
							default -> {
				...
				default -> throw new ParseMetadataException("Person type must be an object or string!", reader);
			}
		}
